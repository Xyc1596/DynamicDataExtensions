...
运行时生成数据，可为每种材料生成单独的配方（可使用本模组动态添加的标签进行匹配），而不需要为每种材料单独编写配方，且自动适配其他模组；
...
动态数据基于原版数据类，生成方式参考了原版数据生成（Data Generator），除通过 Mixin 增加了数据注入步骤外不修改游戏运行逻辑，已测试在 200+ 模组环境中能够正常工作；
