...
import com.xyc.practicalextensions.recipe.cooking.LeatherFromRottenFlesh;
...
public class DynamicRecipeManager {
    public static final Set<IDynamicRecipeModule> MODULES = Set.of(
    ...
    public static Pair<Set<RecipeHolder<Recipe<?>>>, Set<ResourceLocation>> getRecipesToUpdate() {
        ...
        MODULES.stream().filter(IDynamicRecipeModule::condition).forEach(
            m -> {
